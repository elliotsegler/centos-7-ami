...
make_grub_conf $location "(hd0,0)" ttyS0 $uuid
chroot $location/mnt mv /boot/grub/menu.lst /boot/grub/grub.conf
chroot $location/mnt ln -s /boot/grub/grub.conf /boot/grub/menu.lst
install_grub $location/mnt $location/out/$name

install_packages_in_chroot $location $location/mnt

make_fstab $location/mnt/etc/fstab "true" "label"
